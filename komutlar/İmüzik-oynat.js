...
exports.run = async (client, message, args) => {
 ...
 let aktif = db.has(`lang_${message.guild.id}`) === false;
    ...
    .setColor("RANDOM")
    .setDescription("** <:Yorum_x:786578064561012746> You have to write the song you want to listen to! (Song name or Youtube URL)**")
    if (!aktif) if (!args[0]) return message.channel.send(embed);
        
    const voiceChannelAdd = new RichEmbed()
    .setColor("RANDOM")
    .setDescription(`**<:Yorum_x:786578064561012746> Please join any voice channel.**`)
    ...
    if (!permissions.has('CONNECT')) {
      ...
      .setColor("RANDOM")
      .setDescription(`<:Yorum_x:786578064561012746> **I do not have enough permission to join any voice channel.**`)
       if (!aktif)return message.channel.send(warningErr);
    }
    if (!permissions.has('SPEAK')) {
      const musicErr = new RichEmbed()
      .setColor("RANDOM")
      .setDescription(`<:Yorum_x:786578064561012746> I can't turn on music/play songs because I'm not allowed to speak on the channel or my microphone is off.`)
      ...
      try {
          ...
          .setColor("RANDOM")
          .setDescription(`<:Yorum_x:786578064561012746> A song with the name you're looking for has not been found!`) 
    ...
    async function handleVideo(video, message, voiceChannel, playlist = false) {
        ...
        if (!serverQueue) {
          ...
          .setColor("RANDOM")
          .setDescription(`<:Yorum_tik:786578062955118612> [the song ${song.title}](https://www.youtube.com/watch?v=${song.id})  was added to List!`)
